...
// 抓取三大法人區分各選擇權
func CrawlBigThreeOption(ctx context.Context) {
	err := chromedp.Run(ctx,
		chromedp.Navigate("https://www.taifex.com.tw/cht/3/optContractsDate"))
	handleError(err)
	startDate := time.Date(2024, time.January, 3, 0, 0, 0, 0, time.UTC)
	toDate := time.Date(2021, time.January, 4, 0, 0, 0, 0, time.UTC)
	for {
		fmt.Println("[Option] date=", startDate)
		c, err := data.FindBigThreeOptionCountByDate(startDate)
		handleError(err)
		if c != 0 {
		...
		if isTableExist {
			...
			data.InsertBigThreeOption(&objects.BigThreeOption{
				DataDate:                         startDate,
